...
ap.add_argument("-a", "--audio", type=bool, default=True,
	help="whether to play audio or not")
...
while True:
	...
	for i in np.arange(0, predictions.shape[2]):
		...
		if confidence > args["confidence"]:
			...
			b = ""
			c = "j"
			while (not ("b'Distance: " in b and c.isdigit())):
				b = str(ser.readline())
				c = b[11:-5].rstrip().lstrip()
			dist = int(b[11:-5])
			print(dist)
			engine.say(CLASSES[idx] + "is" + str(dist) + "centimetres away")
			engine.runAndWait()
			time.sleep(2)
